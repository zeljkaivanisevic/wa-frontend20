...
watch: {
 "store.searchTerm": _.debounce(function(val) {this.fetchPosts(val)}, 500),
  "store.searchTerm1": _.debounce(function(val) {this.fetchPosts(val)}, 500),
...
fetchPosts() {
 ...
 fetch(`http://localhost:3000/posts?title=${this.store.searchTerm}&createdBy=${this.store.searchTerm1}`)
 .then(response => {
